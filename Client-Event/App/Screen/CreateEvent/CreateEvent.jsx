...
export default function CreateEvent() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch(`${PORT_API}/api/v1/category/all-categories`);
				if (!response.ok) {
			...
			if (formattedTime) {
				if (endTimeText !== "Pick a Time" && endTimeText) {
					...
					if (isValid) {
						...
						alert("Warning: Start time is at least 30 minutes before the end time.");
					}
				} else {
					setStartTimeText(formattedTime);
				}
	...
	const onEndChange = (event, selectedDate) => {
		...
		if (endMode === "time") {
			const startTime = new Date(startDateTime).getHours() * 60 + new Date(startDateTime).getMinutes();
			const endTime = new Date(currentDate).getHours() * 60 + new Date(currentDate).getMinutes();
			if (endTime < startTime + 30) {
				alert("Warning: End time should be at least 30 minutes after the start time.");
			} else {
				const formattedTime = convertToAMPM(currentDate.toLocaleTimeString());
	...
	const renderLabel = () => {
		if (eventType || isFocus) {
			return <Text style={[styles.label, isFocus && { color: "blue" }]}>Categories</Text>;
		}
		return null;
	};
	const closeModal = () => {
	...
	return (
        ...
        >
          <EvilIcons name="navicon" size={35} color={Colors.PRIMARY} />
				...
				<Text style={{ fontSize: 25, fontWeight: "700", color: Colors.PRIMARY }}>Create Event</Text>
			</View>
				...
				<KeyboardAvoidingView>
					...
					<View>
						<TouchableOpacity style={styles.imgContainer} onPress={() => setModalVisible(true)}>
							{images.length > 0 ? (
								<Image source={{ uri: images[0] }} style={{ width: "100%", height: "100%", borderRadius: 8 }} />
							) : (
								...
								<View style={styles.modalContainer}>
									<TouchableWithoutFeedback>
										<View style={styles.modalContent}>
											<Text style={{ fontSize: 22, fontWeight: "bold" }}>Upload Photo</Text>
											<View
												style={{
													width: "100%",
					...
					<View style={{ marginBottom: 12 }}>
						...
						<View style={styles.dateContainer}>
							...
							<TouchableOpacity onPress={showStartTimepicker} style={[styles.button, styles.timeButton]}>
						...
						<View style={styles.dateContainer}>
							<TouchableOpacity
								...
								style={[
									styles.button,
									styles.timeButton,
									startTimeText === "Pick a Time" || startButtonText === "Pick a Date"
					...
					<TouchableOpacity style={{ marginTop: 15, paddingBottom: 40 }} onPress={handleConfirmAndBook}>
